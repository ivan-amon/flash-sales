...
const { organizer, resendOrganizerVerificationEmail } = useAuth()
...
async function handleResend(): Promise<void> {
  ...
  try {
    const ok = await resendOrganizerVerificationEmail()
        ...
        <div class="card">
          <div class="card-body text-center p-4 p-md-5">
            ...
            <p class="text-muted mb-4">
              ...
              <span class="fw-semibold">{{ organizer?.email }}</span>. Click it to activate your
              organizer account before creating events.
            </p>
            ...
            <p class="text-muted mt-3 mb-0">
              Already verified?
              <router-link to="/organizer/events/create">Create an event</router-link>
            </p>
          </div>
        </div>
      </div>
